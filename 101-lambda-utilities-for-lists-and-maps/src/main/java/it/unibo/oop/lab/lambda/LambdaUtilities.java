...
public final class LambdaUtilities {
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        final BiFunction<Set<T>,  Set<T>,  Set<T>> bifunction = (currentSet, newValue) -> {
            final var newSet = new HashSet<>(currentSet);
            newSet.addAll(newValue);
            return newSet;
        };

        list.forEach(t -> {
            map.merge(op.apply(t), Set.of(t), bifunction);
        });
        return map;
    }
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((key, value) -> {
            newMap.put(key, value.orElse(def.get()));
        });
        return newMap;
    }
